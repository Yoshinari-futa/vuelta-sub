...
/**
 * Wallet カード裏の「Information」フィールドを紹介リンクで上書きするための metaData。
 *
 * 現状 PassKit テンプレートに定義済みの back フィールドは `universal.info`（Information）
 * の 1 つだけ。新しい key の backField は passOverrides で送っても反映されないため、
 * この既存フィールドの値を紹介リンクに差し替える方針。
 *
