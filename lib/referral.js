 ...
 */
/**
 * Wallet 裏 Information 欄に表示するメッセージを生成。
 * 誕生月の会員には誕生日メッセージ、それ以外は紹介メッセージ。
 *
 * @param {string} externalId
 * @param {object} [opts]
 * @param {string} [opts.birthMonth] 会員の誕生日（例: "3/15"）
 */
function getReferralMetaData(externalId, opts = {}) {
  const shareUrl = buildShareUrl(externalId);
  if (!shareUrl) return {};

  const { isBirthdayMonth } = require('./birthday');
  const isBday = isBirthdayMonth(opts.birthMonth);
