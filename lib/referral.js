...
/**
 * Wallet カード裏のカスタム Back Field（key: memberInfo）に紹介リンクを表示するための metaData。
 *
 * PassKit の予約フィールド universal.info は userCanSetValue: false の制約で
 * 会員ごとの metaData 値が反映されない仕様だったため、テンプレート側に
 * カスタムフィールド（key: memberInfo, label: Information）を追加して、
 * その値をこの metaData キー経由で動的に上書きする構成。
 *
