 ...
 * PassKitのパスにstrip画像を設定するユーティリティ
 *
 * Step 1: 画像をPassKit APIにアップロード → imageId取得
 * Step 2: tier情報を取得して現状確認
 * Step 3: tierのpassTemplateを更新してstrip画像を設定
 */
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      const tierData = await tierResp.text();
      steps.push({
        step: 'get_tier',
        status: tierResp.status,
        body: tierData.substring(0, 3000),
      });
    } catch (err) {
    ...
    // === Step 2: 画像アップロード ===
    // PassKit image API: POST /image
    let imageId = null;
    const imageUploadPayloads = [
      // Format 1: imageData with strip key
      {
        name: 'imageData.strip',
        body: { imageData: { strip: STRIP_IMAGE_BASE64 } },
      },
      // Format 2: direct base64 with type
      {
        name: 'direct_base64',
        body: { imageType: 'STRIP', imageData: STRIP_IMAGE_BASE64 },
      },
      // Format 3: nested with resolution
      {
        name: 'nested_resolution',
        body: { imageData: { strip: { '1x': STRIP_IMAGE_BASE64 } } },
      },
    ];

    for (const attempt of imageUploadPayloads) {
      if (imageId) break;
      try {
        const resp = await fetch(`${host}/image`, {
          method: 'POST',
          headers: {
            'Authorization': token,
        ...
        steps.push({
          step: `upload_image_${attempt.name}`,
          status: resp.status,
        ...
        if (resp.ok) {
          try {
            ...
            imageId = data.id || data.Id || data.imageId;
            steps.push({ step: 'image_id_found', imageId });
          } catch (e) {
            steps.push({ step: 'parse_image_response_error', error: e.message });
          }
        }
      } catch (err) {
        steps.push({ step: `upload_image_${attempt.name}_error`, message: err.message });
      }
    }

    // === Step 3: もしimageIdが取れたら、tierを更新してstrip画像を設定 ===
    if (imageId) {
      // tier PUT で passTemplate.images.strip を設定
      const updatePayloads = [
        {
          name: 'tier_put_imageIds',
          method: 'PUT',
          url: `${host}/members/tier`,
          body: {
            id: tierId,
            programId: programId,
            passTemplateId: undefined, // tierから取得した値があれば
            imageIds: { strip: imageId },
          },
        },
        {
          name: 'tier_patch_images',
          method: 'PATCH',
          url: `${host}/members/tier`,
          body: {
            id: tierId,
    ...
    // === Step 4: program情報も取得（参考情報） ===
    try {
      const progResp = await fetch(`${host}/members/program/${programId}`, {
        headers: { 'Authorization': token },
      });
      const progData = await progResp.text();
      // imageIds関連の情報だけ抽出
      try {
        const parsed = JSON.parse(progData);
        steps.push({
          step: 'program_image_info',
          imageIds: parsed.imageIds || 'none',
          passTemplateId: parsed.passTemplateId || 'none',
        });
      } catch (e) {
        steps.push({ step: 'program_info_raw', body: progData.substring(0, 500) });
      }
    } catch (err) {
  ...
  res.json({ endpoint: 'setup-strip-image', timestamp: new Date().toISOString(), steps });
};
